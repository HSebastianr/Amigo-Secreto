...
function adicionarAmigo() {
    ...
    if (campoAmigo === '') {
        alert('Incerte el nombre porfavor');
        campoAmigo.focus();
        return;
    }
    if (amigos.includes(campoAmigo)) {
        alert('Amigo ya añadido');
        campoAmigo.focus();
        return;
    }
...
function sortearAmigo() {
    ...
    if (amigos.length < 3) {
        resultado.innerHTML = 'Es Necesario de 3 amigos para sortear';
    } else {
        let amigoEscolhido = amigos[Math.floor(Math.random() * amigos.length)];
        resultado.innerHTML = `El Ganador es: <strong>${amigoEscolhido}</strong>`;
        document.getElementById('amigo').focus();
        return;
    }
